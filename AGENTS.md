...
- **Entry point**: `python gui.py` → `gui.main.main()`. (El README describe `tui.py` con Textual, pero no existe aún.)
...
- **ADVERTENCIA**: `write_log()` en `modulos/common/utils.py` SIEMPRE resuelve a `lab_data/logs/`. NO escribir logs fuera de esa ruta ni en `directorio_pruebas/` ni en el `logs/` raíz.
...
- `assets/modulos/*.png` — 14 iconos PNG 100×100, estáticos y commiteados en el repo

## Documentación

- Cada módulo tiene un `README.md` con teoría profunda. Solo se visualizan desde la GUI (botón Readme) o leyendo el archivo directamente. No hay índice central.
