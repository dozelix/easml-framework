...
Also contains `python/zombie/` — a Minecraft-themed variant with UV evasion,
Manhattan routing, and performance profiling (Python only).

## Repo structure

```
python/creeper/   — Python 3.7+ (interpreted, no build step)
python/zombie/    — Python 3.7+ (Minecraft zombie variant, 7 phases)
...
## Build & run (from within each `*/creeper/` or `python/zombie/` directory)
...
```bash
# From any */creeper/ or python/zombie/ directory:
...
- **Zombie UV evasion**: runs hours 9-17h self-destruct before infection. Run outside business hours for full test.
...
- `python/zombie/README.md` — Zombie variant documentation
